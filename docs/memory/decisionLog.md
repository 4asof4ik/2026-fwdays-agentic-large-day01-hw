...
This log records **major architectural and toolchain choices** visible in the repository. **Entry dates** mark when decisions were **captured in the Memory Bank** (not original upstream adoption dates, unless recoverable from history). Evidence: root and workspace `package.json`, `docs/memory/techContext.md`, `docs/memory/systemPatterns.md`, and source under `excalidraw-app/`, `packages/*`, `examples/*`.
...
* **Consequences**: No npm/pnpm workspaces in use; contributors follow Yarn 1 conventions. Lockfile churn (e.g. registry URL normalization) is a known hygiene topic (see `progress.md`).
...
* **Consequences**: **ESLint** **`no-restricted-imports`** forbids importing **`"jotai"`** directly — code must use **`editor-jotai`** or **`app-jotai`**. Developers must respect store boundaries (see `library.ts` Jotai scoping notes in `progress.md`).
...
* **Consequences**: E2E/browser automation is not fully specified by this stack (see Pending Decisions).
...
* [ ] **Repository hygiene**: Policy for **`repomix-compressed.txt`**, **`yarn.lock`** churn, and **`.cursorignore`** (see `progress.md`).
* [ ] **Course / product scope**: Confirm next milestone — **documentation-only** vs **feature work** in the editor (see `progress.md`).
* [ ] **E2E / browser automation**: Whether to standardize on Playwright, Cypress, or another layer beyond Vitest unit coverage.
* [ ] **Collaboration backends**: Long-term stance on self-hosted vs managed Firebase/socket endpoints (operational, not fully encoded in app code).

---

*Last Updated: 2026-03-26*  
*Verified against source: Yes — `package.json` (root, `excalidraw-app`, `packages/excalidraw`), `.eslintrc.json`, `packages/excalidraw/editor-jotai.ts`, `packages/excalidraw/context/tunnels.ts`, `vitest.config.mts`, `vercel.json`, `Dockerfile`, `docs/memory/techContext.md`, `docs/memory/systemPatterns.md`*
